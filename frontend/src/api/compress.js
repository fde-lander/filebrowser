...
export async function pauseCompress() {
   const apiPath = getApiPath("compress-images/pause", {});
   const response = await fetchURL(apiPath, { method: "POST" });
   if (!response.ok) {
     throw new Error(`Pause API returned ${response.status}`);
   }
   return response.json();
 }

export async function resumeCompress() {
   const apiPath = getApiPath("compress-images/resume", {});
   const response = await fetchURL(apiPath, { method: "POST" });
   if (!response.ok) {
...
export async function cancelCompress() {
   const apiPath = getApiPath("compress-images/cancel", {});
   const response = await fetchURL(apiPath, { method: "POST" });
   if (!response.ok) {
     throw new Error(`Cancel API returned ${response.status}`);
   }
   return response.json();
 }

export async function skipBatch() {
   const apiPath = getApiPath("compress-images/skip-batch", {});
   const response = await fetchURL(apiPath, { method: "POST" });
   if (!response.ok) {
     throw new Error(`Skip API returned ${response.status}`);
   }
   return response.json();
 }

export async function getQueueList() {
   const apiPath = getApiPath("compress-images/queue", {});
   const response = await fetchURL(apiPath, {});
   if (!response.ok) {
